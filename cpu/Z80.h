...
class Z80 : public DebugEventThrower, public ImplementsSaveState
{
 public:
    ...
    void setCycles(u32 c) { cycleCount = c;}
    u32 getCycles() { return cycleCount;}
    u8   getInterruptMode() {return IM;}
    ...
    */
    u16 getAF() { return ( (A << 8) | F);}
    u16 getBC() { return ( (B << 8) | C);}
    u16 getDE() { return ( (D << 8) | E);}
    u16 getHL() { return ( (H << 8) | L);}
    u16 getIX() { return ( IX );}
    u16 getIY() { return ( IY );}
    u16 getPC() { return ( PC );}
    u16 getSP() { return ( SP );}
 ...
 private:
    ...
    */
    void setAF   (u16 v) {A = v>>8; F = v & 0xFF;}
    void setBC   (u16 v) {B = v>>8; C = v & 0xFF;}
    void setDE   (u16 v) {D = v>>8; E = v & 0xFF;}
    void setHL   (u16 v) {H = v>>8; L = v & 0xFF;}

    /*
    * Internal indirect  8 bits regs Addressing.
    * Set/Get Indirect 8 bits values.
    */
    u8   getBCi()       { return env.rd8(getBC());} /* read u8 in (BC) */
    void setBCi(u8 val) { env.wr8(getBC(),val);}    /* write u8 in (BC) */
    u8   getDEi()       { return env.rd8(getDE());} /* read u8 in (DE) */
    void setDEi(u8 val) { env.wr8(getDE(),val);}    /* write u8 in (DE) */
    u8   getHLi()       { return env.rd8(getHL());} /* read u8 in (HL) */
    void setHLi(u8 val) { env.wr8(getHL(),val);}    /* write u8 in (HL) */

    /* 8 bit read/write throught (IX+d) and (IY+d). Warning: PC is not incremented !  */
    u8    getIXdi() {return env.rd8((u16)(IX + (s8)env.rd8(PC)));}
    void  setIXdi(u8 val) { env.wr8((u16)(IX + (s8)env.rd8(PC)),val);}
    u8    getIYdi() {return env.rd8((u16)(IY + (s8)env.rd8(PC)));}
    void  setIYdi(u8 val) { env.wr8((u16)(IY + (s8)env.rd8(PC)),val);}
    void  setNNi16(u16 val16b) {env.wr16(env.rd16(PC),val16b);}
    u16   getNNi16() { return env.rd16(env.rd16(PC));}
    void  setNNi8(u8 val8b) { env.wr8(env.rd16(PC),val8b); }
    u8    getNNi8()   { return env.rd8(env.rd16(PC));}
