...
Z80Dasm::Z80Dasm (Z80Environment &env): env_( env )
{
  //  e = ev;
};

char Z80Dasm::Sign (unsigned char a)
{
    return (a&128)? '-':'+';
}

int Z80Dasm::Abs (unsigned char a)
{
...
/****************************************************************************/
int Z80Dasm::disasm (unsigned PC)
{
     char dest[256];
     char add[32];
     char *S;
	 ...
	 case 0xCB:
	     i++;
	     S=mnemonic_cb[env_.rd8(i++)];
	 break;
	 case 0xED:
	     i++;
	     S=mnemonic_ed[env_.rd8(i++)];
	 break;
	 case 0xDD:
	     i++;
	     r="ix";
		 ...
		 case 0xcb:
	             i++;
	             Offset=env_.rd8(i++);
	             S=mnemonic_xx_cb[env_.rd8(i++)];
		 break;
		 default:
	             S=mnemonic_xx[env_.rd8(i++)];
		 break;
	     }
	 break;
	 case 0xFD:
	 ...
	 {
	     case 'B':
	         sprintf ((char*)buf,"$%02x",env_.rd8(i++));
	         strcat (dest,(char*)buf);
	     break;
	     case 'R':
		 sprintf ((char*)buf,"$%04x",(PC+2+(signed char)env_.rd8(i))&0xFFFF);
		 i++;
		 strcat (dest,(char*)buf);
             break;
	     case 'W':
		 sprintf ((char*)buf,"$%04x",env_.rd8(i)+env_.rd8(i+1)*256);
		 i+=2;
		 strcat (dest,(char*)buf);
	     break;
	     case 'X':
		 sprintf ((char*)buf,"(%s%c$%02x)",r,Sign(env_.rd8(i)),Abs(env_.rd8(i)));
		 i++;
		 strcat (dest,(char*)buf);
             break;
	     case 'Y':
	     ...
	     break;
	     case '!':
	         sprintf (dest,"db     $ed,$%02x",env_.rd8(i-1));
		 sprintf((char*)add,"%.4x", PC);
		 for (unsigned int h=0;h<strlen(dest);h++)
		 {
		     dest[h] = toupper(dest[h]);
		 }

		 for (unsigned int h=0;h<strlen(add);h++)
		 {
		     add[h] = toupper(add[h]);
		 }
		 printf ("0x%s:\t%s\n",add,dest);
	     return i;
	     case '@':
	         sprintf (dest,"db     $%02x",env_.rd8(i-2));
		 sprintf((char*)add,"%.4x", PC);
		 for (unsigned int h=0;h<strlen(dest);h++)
		 {
		     dest[h] = toupper(dest[h]);
	     ...
	     return i-1;
	     case '#':
	         sprintf (dest,"db     $%02x,$cb,$%02x",env_.rd8(i-4),env_.rd8(i-2));
		 sprintf((char*)add,"%.4x", PC);
		 for (unsigned int h=0;h<strlen(dest);h++)
		 {
		     dest[h] = toupper(dest[h]);
